...
#[derive(Clone)]
enum GameCmd {
   Obj(ObjId, String, String), Text(ObjId, String),
...
fn setup_game_script_env(sender : CmdSender, event_r : EventReceiver,
   ...
   let draw_text = move |args_ : Sexps, root : Root, table : EnvId| -> Sexps {
      ...
      let cmd = GameCmd::Text(id, text);
      sender_draw_text.send(cmd).unwrap();
      ...
      for script_cmd in cmds {
         ...
         match script_cmd {
            Text(_, text) => {
               ...
               let rec = Shape::new_builtin(BuiltInShape::Rectangle);
               let m = model_b.shape(rec)
                              .text(text)
                              .color((1.0, 0.0, 0.0, 0.0)) //TODO kk tmp
                              .finalize(&mut game.shader_manager, &game.display);
               let game_obj = GameObject::new(GameObjectType::Model(m));
               game.root.items.push(game_obj);
            ...
            Obj(_, shape_type, color_or_texture) => {
               ...
               let shape = match &*shape_type {
                  ...
                  "triangle-fan" => Shape::new_builtin(BuiltInShape::TriangleFan),
